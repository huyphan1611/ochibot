...
function removePlayerFromList(userId) {
  const gooseDuckMatches = GameMatchesManager.getGooseDuckMatches();

  const index = gooseDuckMatches.findIndex(player => player.userId === userId);
  if (index !== -1) {
    gooseDuckMatches.splice(index, 1);
...
async function updateEmbed(message, timeEnd, userId) {
  ...
  let newTimeAx;
  // const shouldStop = listgooseDuck.findIndex(item => item.userId === userId);
  // if (!shouldStop) {
  if (remainingTime > 0) {
...
async function handleUserLeftVoiceChannel(userId, oldState, newState) {
  ...
  const userIndex = gooseDuckMatches.findIndex(item => item.userId === userId);
  if (userIndex !== -1) {
    const user = gooseDuckMatches[userIndex];
    gooseDuckMatches.splice(userIndex, 1);
...
async function handleVoiceStateUpdate(oldState, newState) {
  ...
  if (oldState.channelId && oldState.channelId !== newState.channelId) {
    const userInGooseDuckMatches = gooseDuckMatches.find(item => item.userId === oldState.id);

    if (userInGooseDuckMatches && userInGooseDuckMatches.voiceChannelLink.includes(oldState.channelId)) {
...
module.exports = {
  ...
  async execute(interaction, client) { // thằng này phải là interaction, client chứ????
    ...
    } else if (interaction.message && interaction.message.content.startsWith('?gooseDuck')) {
...
//     if (existingUserInVC) {
//       await interaction.reply({ content: 'Đã có người trong room xài lệnh /gooseDuck.', ephemeral: true });
//       return;
//     }


    const existingUserIndex = gooseDuckMatches.findIndex(player => player.userId === member.user.id);
    if (existingUserIndex !== -1) {
      clearInterval(countdownIntervals[member.user.id]);
      delete countdownIntervals[member.user.id];
      gooseDuckMatches.splice(existingUserIndex, 1);
    ...
    let embed = new EmbedBuilder()
      .setTitle('<:oz_play:1251569356412813394> 𝗚𝗢𝗢𝗦𝗘 𝗚𝗢𝗢𝗦𝗘 𝗗𝗨𝗖𝗞 ')
      .setDescription(randomDescription)
      .setThumbnail('https://images.sftcdn.net/images/t_app-icon-m/p/e2665680-5e8d-490b-b1c0-a4b3b7b0d8d9/127750113/goose-goose-duck-logo')
      .setColor('#1cf1ef')
      ...
      .addFields([
          ...
          { name: `◜ Slots ◝`, value: `**◟[${slots}/16]◞**`, inline: true }
          ]);
    ...
    if (timeEscapeEnd && Date.now() < timeEscapeEnd) {
      ...
      await interaction.channel.send(`<@&${process.env.gooseDuck_ID}>`);
    ...
    GameMatchesManager.addGooseDuckMatch({
